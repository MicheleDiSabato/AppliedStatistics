...
#meuse.gstat1 <- gstat(id = 'zinc', formula = log(zinc) ~ sqrt(dist),
#                     data = meuse, nmax = 50, model=v.fit)
# v.1 = variogram(meuse.gstat1)
# x11()
# plot(v.1)
# plot(svgm,add=T)
...
s0.new <- as.data.frame(matrix(c(77.6,34.99,1),1,3))
names(s0.new) <- c('lon','lat','D')
coordinates(s0.new) <- c('lon','lat')
predict(g.tr, s0.new)
